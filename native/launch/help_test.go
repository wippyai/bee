...
func TestPlanRefusesMalformedRouteBeforeProjectSelection(t *testing.T) {
	for _, case_ := range []struct {
		...
		{[]string{"hive"}, "bee hive takes invite [--out FILE | --share DIR], invites, peers, revoke INVITE_ID, join INVITE or leave NODE"},
		...
		{[]string{"hive", "invite", "now"}, "bee hive invite takes only --out FILE or --share DIR, not \"now\""},
		{[]string{"hive", "invite", "--out"}, "bee hive invite --out requires a path"},
		{[]string{"hive", "invite", "--out", "a", "--share", "b"}, "bee hive invite takes either --out FILE or --share DIR"},
