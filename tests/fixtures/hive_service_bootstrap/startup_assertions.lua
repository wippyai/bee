...
-- These checks observe the actual lifecycle frame without replacing its scope.
    ...
    assert(actor and actor:id() == "bee.hive_host.supervisor", "wrong service actor")
    assert(security.can("process.registry.register", "bee.hive_host.supervisor"), "missing own-name authority")
    assert(security.can("funcs.call", "bee.hive_host.supervisor:execute"), "missing dispatch authority")
    ...
    assert(not security.can("db.get", "bee.environment:workspace_db"), "unexpected database authority")
