...
STORE_NAMES = ("workspace", "threads", "approvals", "resources", "credentials", "placement", "gateway")
STATE_ENVIRONMENT = {f"BEE_{name.upper()}_DB" for name in STORE_NAMES} | {"BEE_PLACEMENT_ROOT"}


class NativeDesktop(Desktop):
    def __init__(self, binary, folder, state, application=None, arguments=()):
        ...
        env.update(TERM="xterm-256color", HOME=str(folder), PATH=f"{folder}/bin:/usr/bin:/bin")
