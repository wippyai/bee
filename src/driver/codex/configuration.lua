...
M.MAX_URL_BYTES = 512
type Provider = {ref: string, name: string, base_url: string, model: string, loopback_fixture: boolean, digest: string}
...
function M.decode(ref: string, entry: {[string]: unknown}): (Provider?, string?)
    ...
    local unknown_field = bounds.fields(data, {"schema_revision", "name", "base_url", "model", "loopback_fixture"})
    ...
    return {ref = ref, name = name, base_url = base_url, model = model, loopback_fixture = loopback, digest = digest}, nil
end
...
function M.render(provider: Provider, gateway_section: string?): string
    local content = table.concat({
        ...
        "env_key = " .. toml_string(M.ENV_KEY),
        "wire_api = " .. toml_string(M.WIRE_API),
        "",
    }, "\n")
