...
# Governance will be the only admitted agent/app-facing registry writer. Until
# its executor is reviewed, NO shipped policy may grant this authority. A plugin
# or trait adding a broad grant must fail the source/pack admission audit too.
registry_write_actions = {"registry.apply", "registry.apply_version", "registry.overlay.apply"}
for identity, entry in entries.items():
    if entry["kind"] != "security.policy":
        continue
    policy = entry.get("policy", {})
    if policy.get("effect") != "allow":
        continue
    for pattern in policy.get("actions", []):
        assert not any(fnmatchcase(action, pattern) for action in registry_write_actions), ("Unadmitted registry writer", identity, pattern)
assert registry_write_actions <= set(entries["bee:app_boundary_policy"]["policy"]["actions"])
assert entries["bee:app_boundary_policy"]["policy"]["effect"] == "deny"
assert {i for i, e in entries.items() if e["kind"] == "terminal.host"} == {"bee:terminal"}
assert {i for i,e in entries.items() if e["kind"] == "db.sql.sqlite"} == {"bee:workspace_db", "bee:client_db", "bee.threads:db", "bee.placement.native:db", "bee.resources:db", "bee.credentials:db", "bee.approvals:db", "bee.gateway:db", "bee.node:db", "bee.governance:db"}
...
allowed = {"bee", "bee.applications", "bee.desktop", "bee.protocol", "bee.host", "bee.interaction", "bee.launch",
           "bee.node", "bee.sync", "bee.governance",
