...
'''
    ...
    storage("allocate", string.rep("a", 32), "OK", 0)''', 1)
        ...
        for packed in (False, True):
            ...
            args += [command] + ([fixture_mode] if transfer_probe else (["shared-store"] if shared_store else (["session-failed-upgrade"] if failed_session_upgrade else (["session-upgrade"] if session_upgrade else (["client-upgrade"] if client_upgrade else []))))) + [ "--host", "bee:workers", "--set", f"registry.history_path={folder / 'registry.db'}"]
            try:
            ...
            if command in ("retained-supervisor-probe", "retained-client-upgrade-probe"):
