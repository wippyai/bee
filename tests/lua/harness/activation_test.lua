...
local function define_tests()
    test.describe("Harness activation declaration", function()
        test.it("decodes a bounded, distinct binding selection", function()
            local decoded, err = activation.decode_activation("test:activation", entry({schema_revision = "bee.harness-activation@1", bindings = {"bee.driver.claude:binding", "bee.driver.codex:binding"}}))
        ...
        test.it("rejects missing, malformed and broadened declarations", function()
            local missing, missing_error = activation.decode_activation("test:activation", entry(nil))
            test.is_nil(missing)
            test.eq(missing_error, "test:activation has no data")
            local old, old_error = activation.decode_activation("test:activation", entry({bindings = {}}))
            test.is_nil(old)
            test.eq(old_error, "test:activation: schema_revision must be bee.harness-activation@1")
            local duplicate, duplicate_error = activation.decode_activation("test:activation", entry({schema_revision = "bee.harness-activation@1", bindings = {"bee.driver.claude:binding", "bee.driver.claude:binding"}}))
            test.is_nil(duplicate)
            test.eq(duplicate_error, "test:activation: bindings: list item 2 repeats")
            local unknown, unknown_error = activation.decode_activation("test:activation", entry({schema_revision = "bee.harness-activation@1", bindings = {}, publish = true}))
            test.is_nil(unknown)
