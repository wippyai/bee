...
type Resume = {view_id: string, instance_id: string, schema: string, state: string}
local M = {}
function M.select(saved: {records.Record}, live: inventory.State, definition_id: string,
    reserved: {[string]: boolean}): Resume?
    for _, record in ipairs(saved) do
        if record.definition_id == definition_id and not reserved[record.instance_id] then
            local running = false
            ...
            if not running then
                return {view_id = record.id, instance_id = record.instance_id,
