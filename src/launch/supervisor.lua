...
local function run_supervisor(client: string, workspace: unknown, database_resource: string?, retained_owner: string?, initial_application: string?)
    ...
    local function run()
        ...
        local function detach_attachment(resource: desktops.Desktop, id: string?, recipient: string)
            local grants = resource.grants
            if not ((grants.controller and grants.controller.recipient == recipient) or grants.observers[recipient]) then return end
        ...
        "bee.security.desktop:client_workspace_catalog_call_policy", "bee.security.storage:workspace_catalog_read_policy"}) do
                ...
                local started, start_error = desktops.start(retained,
                    {host = host, workspace_id = workspace_id, database = "bee.env:client_db", width = 100, height = 32,
