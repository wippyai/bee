...
local function no_database()
    local handle, err = sql.get("bee.environment:client_db")
...
local function main(mode: string)
    no_database()
    local list = {version = 1, database_resource = "bee.environment:client_db"}
    local allocate = {version = 1, database_resource = "bee.environment:client_db", desktop_id = string.rep("e", 32)}
    if mode == "none" then
        call("list_desktops", list, "DENIED")
        call("allocate_desktop", allocate, "DENIED")
    elseif mode == "capacity" then
        for index = 1, 31 do
            call("allocate_desktop", {version = 1, database_resource = "bee.environment:client_db", desktop_id = string.format("%032x", index)}, "OK")
        end
        count(call("list_desktops", list, "OK"), 33)
        call("allocate_desktop", {version = 1, database_resource = "bee.environment:client_db", desktop_id = string.rep("f", 32)}, "CAPACITY")
        call("allocate_desktop", allocate, "OK")
        count(call("list_desktops", list, "OK"), 33)
    elseif mode == "reader" then
        count(call("list_desktops", list, "OK"), 2)
        call("allocate_desktop", {version = 1, database_resource = "bee.environment:client_db", desktop_id = string.rep("f", 32)}, "DENIED")
        count(call("list_desktops", list, "OK"), 2)
    else
        assert(mode == "seed" or mode == "verify")
        local default = count(call("list_desktops", list, "OK"), mode == "seed" and 1 or 2)
        call("allocate_desktop", {version = 1, database_resource = "bee.environment:client_db", desktop_id = default}, "CONFLICT")
        ...
        call("list_desktops", {version = 1, database_resource = "bee.environment:client_db", actor = "owner"}, "INVALID_ARGUMENT")
        call("allocate_desktop", {version = 1, database_resource = "bee.environment:client_db", desktop_id = "bad"}, "INVALID_ARGUMENT")
        call("allocate_desktop", {version = 1, database_resource = "bee.environment:client_db", desktop_id = allocate.desktop_id, label = "unexpected"}, "INVALID_ARGUMENT")
    end
    no_database()
end
return {main = main}
