...
local function define_tests()
    test.describe("Workspaces frame", function()
        ...
        test.it("draws the create flow's folder step at every size class", function()
            ...
            folder_picker.apply_roots(form.picker, ok({roots = {{root_ref = "bee:workspace_root", access = "write"}, {root_ref = "bee:archive", access = "read"}}}))
            for _, size in ipairs({{40, 12}, {80, 24}, {120, 36}, {160, 48}}) do
            ...
            folder_picker.apply_folders(form.picker, ok({root_ref = "bee:workspace_root", path = "", access = "write",
        ...
        test.it("draws the details step with the new folder only under a writable root", function()
            local form = creation.new()
            folder_picker.apply_roots(form.picker, ok({roots = {{root_ref = "bee:workspace_root", access = "write"}}}))
            folder_picker.open(form.picker)
            folder_picker.apply_folders(form.picker, ok({root_ref = "bee:workspace_root", path = "", access = "write", folders = {{name = "alpha"}}}))
            folder_picker.open(form.picker)
            folder_picker.apply_folders(form.picker, ok({root_ref = "bee:workspace_root", path = "alpha", access = "write", folders = {}}))
            creation.use(form)
            ...
            folder_picker.apply_folders(fixed.picker, ok({root_ref = "bee:archive", path = "", access = "read", folders = {}}))
            creation.use(fixed)
