...
local function define_tests()
    test.describe("Node workspace catalog", function()
        test.it("serves the classic folder workspace by its root and by its identity", function()
            ...
            test.eq(#rows, 1)
            test.eq(rows[1].root_ref, "bee.environment:workspace_root")
        ...
        test.it("refuses selections that name no active catalog row", function()
            ...
            local _, ambiguous = store.open(NODE, {workspace_id = string.rep("a", 32), root_ref = "bee.environment:workspace_root", subpath = ""})
            ...
            local _, escaped = store.open(NODE, {root_ref = "bee.environment:workspace_root", subpath = "../other"})
