...
local function await_receipt(thread_id: string, attempt_id: string): {[string]: unknown}
    local last_status: unknown = nil
    local last_error = "not observed"
    local last_records: {string} = {}
    for _ = 1, 600 do
        local observed, status = pcall(function(): {[string]: unknown}
            return call("bee.placement.native:status", {attempt_id = attempt_id})
        end)
        if observed then last_status = status else last_error = tostring(status) end
        local read_ok, kinds = pcall(records, thread_id)
        if read_ok then last_records = kinds :: {string} end
        if observed and count(last_records, "receipt") == 1 then
    ...
    local status_json = json.encode(last_status)
    local evidence_ok, evidence = pcall(function(): {[string]: unknown}
        return call("bee.placement.native:evidence", {attempt_id = attempt_id, limit = 64})
    end)
    local evidence_json = evidence_ok and json.encode(evidence) or tostring(evidence)
    error("profile attempt did not retain its receipt: status=" .. tostring(status_json or last_error)
        .. " records=" .. table.concat(last_records, ",") .. " evidence=" .. tostring(evidence_json))
end
    ...
    test.describe("Named profile policy environment isolation", function()
        test.it("runs overlapping named definitions for one Claude batch profile with distinct policy environments", function()
            ...
            local ok, failure = pcall(function()
                ...
                local resource_roots_data = resource_roots.data :: {[string]: unknown}
                local copied_resource_roots: {{[string]: unknown}} = {}
                local found_resource_root = false
                for index, item in ipairs(resource_roots_data.roots :: {{[string]: unknown}}) do
                    copied_resource_roots[index] = item
                    if item.root_ref == ROOT then found_resource_root = true end
                end
                if not found_resource_root then
                    copied_resource_roots[#copied_resource_roots + 1] = {root_ref = ROOT, access = "write"}
                end
                resource_roots.data = {roots = copied_resource_roots}
                ...
                call("bee.resources:associate", {workspace_id = workspace, name = "project", root_ref = ROOT, subpath = "", allowed_access = "write"})
