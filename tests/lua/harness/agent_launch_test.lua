...
local function define_tests()
    test.describe("Agent launch owner operation", function()
        ...
        test.it("decodes a working directory, thread, placement and saved profile choice", function()
            ...
            local function with(field: string, value: unknown): (agent_launch.Request?, string?)
                local request: Object = {}
                for key, item in pairs(base) do request[key] = item end
                request[field] = value
                return agent_launch.decode_request(request)
            end
            local resource = with("workdir", {resource = "project"})
            test.eq(resource and resource.workdir and resource.workdir.resource, "project")
            local folder = with("workdir", {root_ref = "bee:workspace_root", path = "legacy/app"})
            test.eq(folder and folder.workdir and folder.workdir.path, "legacy/app")
            local root = with("workdir", {root_ref = "bee:workspace_root"})
            test.eq(root and root.workdir and root.workdir.path, "")
            test.eq(select(2, with("workdir", {resource = "project", root_ref = "bee:workspace_root"})), "workdir names either a resource or a root_ref and path")
            test.eq(select(2, with("workdir", {root_ref = "bee:workspace_root", path = "../up"})), "workdir.path: subpath has an invalid segment")
            ...
            local profiled = agent_launch.decode_request({definition_ref = PERMITTED, brief = "x", idempotency_key = "k", saved_profile_id = "p", saved_profile_revision = 2})
