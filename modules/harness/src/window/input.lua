-- MIT. Narrow the untyped select value back to the runtime terminal event.
...
function M.decode(value: unknown): tty.TTYEvent?
    ...
    if value.type == "key" then
        if type(value.key) ~= "string" or type(value.key_type) ~= "string" or (value.action ~= "press" and value.action ~= "release") then return nil end
        return {type = "key", key = value.key, key_type = value.key_type, action = value.action == "release" and "release" or "press", ctrl = value.ctrl == true, alt = value.alt == true, shift = value.shift == true}
