...
local function define_tests()
    test.describe("owner stop", function()
        ...
        test.it("accepts the forwarded stop only from this node's supervisor", function()
            test.is_true(owner_stop.from_supervisor("{node@bee.hive_host:supervisor_host|0x1}", "{node@bee.hive_host:supervisor_host|0x1}"))
            test.is_false(owner_stop.from_supervisor("{node@bee:workers|0x2}", "{node@bee.hive_host:supervisor_host|0x1}"))
            test.is_false(owner_stop.from_supervisor("{node@bee.hive_host:supervisor_host|0x1}", nil))
        end)
    end)
end
