...
local function define_tests()
    test.describe("Retained workspace names", function()
        test.it("keys the owner route and bridge by the workspace selection", function()
            ...
            test.eq(#classic, 32)
            test.eq(workspaces.key({root_ref = "bee.environment:workspace_root", subpath = ""}), classic)
            local other_root = assert(workspaces.key({root_ref = "bee.environment:workspace_root", subpath = "projects/one"}))
            ...
            test.is_nil(workspaces.key({root_ref = "bee.environment:workspace_root"}))
