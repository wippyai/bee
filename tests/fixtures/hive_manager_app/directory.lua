...
type Directory = {supervisor: (Directory) -> Supervisor, members: (Directory) -> ({Member}, string?), presence: (Directory, string) -> Reply, stats: (Directory, string) -> Reply, workspaces: (Directory, string, Query) -> Catalog, attach: (Directory, Attach) -> Outcome}
function M.live(_value: unknown): Directory
    ...
    local function workspaces(_: Directory, _node: string, _query: Query): Catalog
        if MODE == "slow" then
        ...
        if MODE == "unavailable" then return {available = false, reason = "No supervisor is available", owner_generation = "", workspaces = none} end
        return {available = true, reason = "", owner_generation = "test-generation", workspaces = {
            {workspace_id = FIRST, label = "main", served = true}, {workspace_id = SECOND, label = "replacement", served = false}}}
    end
