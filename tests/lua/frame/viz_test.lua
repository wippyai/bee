...
local function define_tests()
    test.describe("Visualization kit", function()
        test.it("keeps a live series bounded and ordered oldest first", function()
            ...
            test.is_true(viz.is_gap(viz.latest(series) or 0))
        end)
        test.it("draws at most one frame per cadence interval", function()
            local cadence = viz.cadence(1000)
            test.is_true(viz.due(cadence, 5000))
            test.is_false(viz.due(cadence, 5400))
            test.is_true(viz.due(cadence, 6000))
            test.is_false(viz.due(cadence, 6999))
            test.is_true(viz.due(cadence, 7100))
            test.eq(cadence.next_at, 8000)
            test.is_true(viz.due(cadence, 12345))
        ...
        test.it("draws line and area charts with axis labels", function()
            ...
            viz.line(line, {x = 2, y = 1, width = 38, height = 7}, {{values = wave()}}, {unit = "ms", from = "-60s", to = "now"})
            golden(line, LINE)
            ...
            viz.line(area, {x = 2, y = 1, width = 38, height = 5}, {{values = wave()}}, {area = true, min = 0, max = 100})
            golden(area, AREA)
        end)
        ...
        test.it("draws stat tiles, a timeline and a legend", function()
            ...
            test.eq(viz.tiles(tiles, {x = 2, y = 1, width = 58, height = 3}, {{label = "Heap", value = "12.4 MiB", note = "▲0.8", values = wave()},
