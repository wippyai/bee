...
local function define_tests()
    ...
    test.describe("Native interactive launch specifications", function()
        test.it("opens native UIs without a prompt and preserves scoped flags on resume", function()
            ...
            test.eq(quote.line(claude_window.argv), "claude --permission-mode default --model sonnet --effort high")
            ...
            test.eq(quote.line(codex_window.argv), "codex --sandbox workspace-write")
            ...
            test.eq(quote.line(codex_launch.specification(resume).argv), "codex --profile bee --sandbox read-only resume native-session -- --help")
            claude_request.brief = "--help"
            claude_request.resume_ref = "native-session"
            test.eq(quote.line(claude_launch.specification(claude_request).argv), "claude --permission-mode default --model sonnet --effort high -r native-session -- --help")
        end)
    ...
    test.describe("Codex exec-json normalization", function()
        ...
        test.it("reports a stream without turn.completed as uncertain and builds argv", function()
            ...
            test.eq(quote.line(resumed.argv), "codex --profile bee --sandbox read-only exec resume sess-1 --json --skip-git-repo-check -")
