...
revision. A grant binds the authenticated subject, an audience (the
...
|---|---|
| `bee.resources` | `authority`: the six operations and the store opened through `bee.persist`; `resources`: linked references (store, host roots); contract `contract` with binding `local` |
...
placement service entries). Revocation stops future authorization; an
