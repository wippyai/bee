...
A definition with `agent_ref` runs the referenced framework agent through a
CLI harness route (Claude, Codex, agy, Grok or Muse). Admission resolves the
closure from the same snapshot as the driver and policy, checks the route's
capability parity, and pins the closure digest in the plan; a changed agent,
trait, tool, delegate or contract reference returns `CONFLICT` on the next
admission. The route honors the exact agent identity, the composed prompt and
context, the selected trait and tool schemas, the admitted delegates and the
host-approved model mapping from the launch policy's `agent_model_map`. A
tuning hint passes only when the agent owner lists it as declinable;
required hooks, memory, wrappers, contracts, options and unknown fields are
refused (`UNSUPPORTED_CAPABILITY`, `INVALID`), and a trait is never reduced
to its prompt. Delegates outside the policy's `agent_delegates` are refused
(`FORBIDDEN`). The closure's composed text travels through the profile
instruction channel, so an agent route needs `profile_instructions`; a saved
profile may only narrow inside the closure and never claims the model option.
