-- MIT. Exact owner-feed operations through the existing admitted Hive route.
...
local M = {}
M.OPERATIONS = {
    ["bee.node:describe"] = "bee.node", ["bee.node:update_metadata"] = "bee.node",
    ["bee.node:snapshot"] = "bee.node", ["bee.node:read_after"] = "bee.node",
    ["bee.approvals:feed_snapshot"] = "bee.approvals", ["bee.approvals:feed_read_after"] = "bee.approvals",
    ["bee.approvals:read"] = "bee.approvals", ["bee.approvals:decide"] = "bee.approvals",
    ["bee.approvals:withdraw"] = "bee.approvals",
}
...
function M.handle(value: unknown): types.Reply
    ...
    local service = M.OPERATIONS[request.operation_ref]
    if not service then return denied(id, "DENIED", "operation is not admitted by the feed route") end
    ...
    if verdict_error then logger:named("bee.hive.feed"):error("Feed invocation check failed", {cause = tostring(verdict_error):sub(1, 1024)}) end
