...
function M.check(candidate: Candidate, context: Context): (Report?, string?)
    ...
    -- A package that owns kernel definitions in the base, or kernel code in
    -- the overlay being replaced, cannot be updated: the complete-set update
    -- would replace or remove them. Host records the materializer composes
    -- into an application's own overlay are regenerated by the host and do
    -- not make that application part of the kernel.
    local kernel_packages: {[string]: boolean} = {}
    for id in pairs(kernel) do
        local base = context.entries[id]
        local overlay = context.installed_entries and context.installed_entries[id] or nil
        if base then kernel_packages[base.package] = true
        elseif overlay and protected_kernel.follows(overlay.kind) then kernel_packages[overlay.package] = true end
    end
