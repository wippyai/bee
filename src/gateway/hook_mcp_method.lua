...
local function handle(): nil
    ...
    if not host:find("^127%.0%.0%.1:%d+$") and not host:find("^localhost:%d+$") then answer(response, http.STATUS.FORBIDDEN, mcp.failure(nil, mcp.INVALID_REQUEST, "host is not the loopback listener")); return nil end
