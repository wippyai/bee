            ...
            for run in runs:
                if run.poll() is None:
                    try:
                        ...
                        run.kill()
                        run.wait()
    print("Gateway slices 1, 2 and hook endpoints: agent-requested access through the durable approval inbox, consumed-effect handoff recovery, binding-isolated grants, request/status replay, denied decisions and preserved deselection; configurable traits with stable dispatch, fixed/dynamic native context, binding isolation, concurrent selection CAS and revocation; authenticated caller-owned workspace create/edit/freeze with idempotent replay and cross-actor denial; explicitly admitted thread_message append with bound context, idempotent replay and conflict, default read-tool profile preservation; thread_sessions listing only the workspace's running sessions the subject reads, thread_message to a session by action, attempt or thread addressed to its action and naming the sender's, unreachable sessions refused as not found, and thread_notify registering once and telling the caller on its own thread when the session's turn ends; hook credentials separate from tool credentials, empty-body answers, occurrence identity with replay and conflict, "
