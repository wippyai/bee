...
| Launch policies | `bee.launch_policy` (`executables`, `environment`, `prepare_options`, `permission_exchange`, `codex_provider_ref`, `gateway_tools`, `gateway_hooks`, `gateway_ttl_ms`, cleanup and timing) | The host's authority over how a launch runs: bound executable, nonsecret environment, driver options, gateway |
