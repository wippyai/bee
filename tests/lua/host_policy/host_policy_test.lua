...
local function evaluate_host_policy(policy_name: string, nonhost_action: string, nonhost_resource: string)
    ...
    test.is_false(call_can(scope, "process.host", "bee.hive_host:supervisor_host"))
...
local function define_tests()
    test.describe("Host policy isolation and boundary enforcement", function()
        test.it("allows the native Hive supervisor to release the eventual name it publishes", function()
            ...
            test.is_true(call_can(scope, "process.registry.register.eventual", "bee.hive_host.supervisor/Antares"))
            test.is_true(call_can(scope, "process.registry.unregister.eventual", "bee.hive_host.supervisor/Antares"))
            test.is_false(call_can({"bee.security:base_app_policy"}, "process.registry.unregister.eventual", "bee.hive_host.supervisor/Antares"))
        end)
        test.it("limits waiter startup to its own name and reply delivery", function()
            ...
            test.is_false(call_can(scope, "process.registry.register", "bee.hive_host.supervisor"))
            ...
            test.is_false(call_can(scope, "process.spawn", "bee.hive_host.supervisor:main"))
        ...
        test.it("limits Hive desktop bootstrap to its selected policies", function()
            local scope = {"bee.hive_host.desktop:host_policy"}
            ...
            test.is_false(call_can(scope, "db.get", "bee.environment:client_db"))
        ...
        test.it("evaluates reconstructed host scope (host_policy + host_spawn_policy + workspace_storage_policy)", function()
            ...
            test.is_true(call_can(scope, "db.get", "bee.environment:workspace_db"))
            test.is_false(call_can(scope, "db.get", "bee.environment:client_db"))
        end)
        ...
        test.it("defeats broad fixture spawn allow via core_spawn_boundary explicit deny", function()
            ...
            for _, action in ipairs({"process.spawn", "process.spawn.monitored", "process.spawn.linked", "process.exec"}) do
                test.is_false(call_can(scope, action, "bee.hive_host.supervisor:main"))
                test.is_false(call_can(scope, action, "bee.hive:supervisor"))
                test.is_false(call_can(scope, action, "bee.hive_host:supervisor_host"))
