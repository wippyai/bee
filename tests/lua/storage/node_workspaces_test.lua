...
local function define_tests()
    test.describe("Node workspace catalog", function()
        test.it("serves the classic folder workspace by its root and by its identity", function()
            ...
            test.eq(#rows, 1)
            test.eq(rows[1].root_ref, "bee.env:workspace_root")
        ...
        test.it("refuses selections that name no active catalog row", function()
            ...
            local _, ambiguous = store.open(NODE, {workspace_id = string.rep("a", 32), root_ref = "bee.env:workspace_root", subpath = ""})
            ...
            local _, escaped = store.open(NODE, {root_ref = "bee.env:workspace_root", subpath = "../other"})
    ...
    test.describe("Folder workspace composition", function()
        test.it("holds no workspace in a fresh node database until the folder is opened", function()
            ...
            test.eq(#query(resource, "SELECT id FROM workspace_schema_migrations"), 9)
        ...
        test.it("keeps the folder workspace of an upgraded node catalog", function()
            ...
            local old_row = assert(db:query("SELECT workspace_id FROM workspaces"))[1]
            local old_id = old_row.workspace_id
            assert(db:execute("INSERT INTO workspace_state (workspace_id, schema_version, generation, value, updated_at) VALUES (?, 1, 2, ?, 'before')",
                {old_id, '{"version":1,"applications":[{"definition_id":"bee.inbox:app"}],"opaque":"bee.inbox:app"}'}))
            assert(db:execute("INSERT INTO workspace_application_thread_bindings (workspace_id, instance_id, thread_id, definition_id, actor_id, role, binding_revision, state, idempotency_key, definition_revision, initiating_owner_id, gateway_binding_id, gateway_approval_id, gateway_proposal_digest, access, join_expected_revision, cleanup_pending) VALUES (?, 'old-instance', 'thread', 'bee.inbox:app', 'actor', 'participant', 1, 'revoked', 'old-key', 'rev', 'owner', 'binding', 'approval', ?, 'observe_post', 1, 0)", {old_id, string.rep("a", 64)}))
            db:release()
            local seeded = query(resource, "SELECT workspace_id FROM workspaces WHERE root_ref = 'bee.environment:workspace_root' AND subpath = ''")
            test.eq(#seeded, 1)
