...
function M.main()
    ...
    assert(process.send(endpoint, "bee.app.request", {version = 1, request_id = "open-neighbor",
        op = "open", workspace_id = workspace_id, definition_id = "bee.console:app"}))
    local neighbor_opened = reply("open-neighbor", "open")
    assert(neighbor_opened.error_code == "" and neighbor_opened.mount ~= "")
    local neighbor, neighbor_error = tty.attach(neighbor_opened.mount)
    if not neighbor then error(tostring(neighbor_error)) end
    command(neighbor, "printf 'BEE_NEIGHBOR_%s\\n' \"$$\"")
    local neighbor_pid = wait_for(neighbor, "BEE_NEIGHBOR_(%d+)")
    assert(neighbor_pid ~= native_pid)
    bind("wrong-instance", "", opened.id, neighbor_opened.instance_id)
    assert(reply("wrong-instance", "bind").error_code == "not_found")
    assert(first:snapshot(), "Wrong identity revoked the live controller")
    bind("detach-terminal", "", opened.id, opened.instance_id)
    ...
    command(neighbor, "printf 'BEE_UNDISTURBED_%s\\n' \"$$\"")
    wait_for(neighbor, "BEE_UNDISTURBED_" .. neighbor_pid)
    time.sleep("400ms")
    bind("second-terminal", owner, opened.id, opened.instance_id)
    ...
    assert(process.send(endpoint, "bee.app.request", {version = 1, request_id = "other-client-terminal",
        op = "open", workspace_id = workspace_id, definition_id = "bee.console:app"}))
    local other = reply("other-client-terminal", "open")
    assert(other.error_code == "")
    bind("other-client-bind", consumer, other.id, other.instance_id)
    local other_mount = reply("other-client-bind", "attached")
    assert(other_mount.error_code == "" and reply("other-client-bind", "bind").error_code == "")
    assert(process.send(consumer, "bee.controller.command", other_mount.mount))
    consumer_status("attached")
    assert(process.send(endpoint, "bee.app.request", {version = 1, request_id = "unbind-client",
        op = "unbind", workspace_id = workspace_id, recipient = owner}))
    assert(reply("unbind-client", "unbind").error_code == "")
    local old_frame, old_error = second:snapshot()
    assert(not old_frame and old_error, "Client detach retained first grant")
    old_frame, old_error = neighbor:snapshot()
    assert(not old_frame and old_error, "Client detach retained second grant")
    assert(process.send(consumer, "bee.controller.command", "verify"))
    consumer_status("verified")
    assert(process.send(endpoint, "bee.app.request", {version = 1, request_id = "after-unbind",
        op = "open", workspace_id = workspace_id, definition_id = "bee.console:app"}))
    local after_unbind = reply("after-unbind", "open")
    assert(after_unbind.error_code == "" and after_unbind.mount == "", "Detached client remained the default recipient")
    assert(process.send(consumer, "bee.controller.command", "stop"))
    consumer_status("closed")
    first:close(); second:close(); neighbor:close()
    ...
    process.unlisten(replies); process.unlisten(catalogs); process.unlisten(controller_status)
end
return M
