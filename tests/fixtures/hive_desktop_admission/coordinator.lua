...
local function main(node: string)
    ...
    if node == "node-0" or node == "node-1" then
        if node == "node-0" then
            local registered, register_error = process.registry.register("bee.desktop.admission.probe.release")
    ...
    while true do
        ...
        elseif command == "stop" then
            if releases then process.unlisten(releases); process.registry.unregister("bee.desktop.admission.probe.release") end
