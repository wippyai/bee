...
local function define_tests()
    test.describe("Gateway hooks", function()
        ...
        test.it("shapes bounded identified inbox context for hook boundaries", function()
            test.eq(hook_inbox.supported("UserPromptSubmit"), true)
            test.eq(hook_inbox.supported("Stop"), true)
            test.eq(hook_inbox.supported("PreToolUse"), false)
            test.eq(hook_inbox.supported(nil), false)
            local items = {
                {record_id = "record-1", message_id = "message-1", inbox_sequence = 1, sender_action_id = "sender", content = {text = "hello"}},
                {record_id = "record-2", message_id = "message-2", inbox_sequence = 2, sender_action_id = "sender", content = {text = string.rep("x", 2000)}},
            }
            local text = hook_inbox.format(items, false)
            test.is_true(text:find("record-1", 1, true) ~= nil)
            test.is_true(text:find("message-2", 1, true) ~= nil)
            test.is_true(text:find("sequence 2", 1, true) ~= nil)
            test.is_true(text:find("sender", 1, true) ~= nil)
            test.is_true(text:find("hello", 1, true) ~= nil)
            test.is_true(text:find("truncated", 1, true) ~= nil)
            test.is_true(#text < 2048)
            test.eq(hook_inbox.format({}, false), "")
            test.is_true(hook_inbox.format(items, true):find("more waiting", 1, true) ~= nil)
            test.is_nil(hook_inbox.format(items, false):find("more waiting", 1, true))
        end)
        test.it("shapes per-driver hook responses without inventing control fields", function()
            test.eq(hook_inbox.carry_text("UserPromptSubmit", "ctx"), "ctx")
            test.is_nil(hook_inbox.carry_text("Stop", "ctx"))
            test.is_nil(hook_inbox.carry_text("UserPromptSubmit", nil))
            test.is_nil(hook_inbox.carry_text("PreToolUse", "ctx"))
            local body = hook_inbox.http_body("ctx")
            if not body then error("Claude hook body is missing") end
            test.eq(body.additionalContext, "ctx")
            test.is_nil(hook_inbox.http_body(nil))
            test.is_nil(hook_inbox.http_body(""))
        end)
