...
local function define_tests()
    test.describe("Agent launch owner operation", function()
        ...
        test.it("decodes a working directory, thread, placement and saved profile choice", function()
            ...
            local folder = with("workdir", {root_ref = "bee.environment:workspace_root", path = "legacy/app"})
            test.eq(folder and folder.workdir and folder.workdir.path, "legacy/app")
            local root = with("workdir", {root_ref = "bee.environment:workspace_root"})
            test.eq(root and root.workdir and root.workdir.path, "")
            test.eq(select(2, with("workdir", {resource = "project", root_ref = "bee.environment:workspace_root"})), "workdir names either a resource or a root_ref and path")
            test.eq(select(2, with("workdir", {root_ref = "bee.environment:workspace_root", path = "../up"})), "workdir.path: subpath has an invalid segment")
