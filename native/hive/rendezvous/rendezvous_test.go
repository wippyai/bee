...
func TestDescriptorSupervisorAddress(t *testing.T) {
	...
	base.Supervisor = "{forge@bee.hive.service:supervisor_host|0x1}"
	if err := base.validate(); err != nil {
		t.Fatalf("valid supervisor address refused: %v", err)
	}
	for _, bad := range []string{
		"{other@bee.hive.service:supervisor_host|0x1}",
		"{forge@bee:workers|0x1}",
		"{forge@bee.hive.service:supervisor_host|}",
		"not-a-pid",
	} {
		invalid := base
...
func TestDescriptorDecodesPublishedSupervisorAddress(t *testing.T) {
	published := sample()
	published.Supervisor = "{forge@bee.hive.service:supervisor_host|0x1}"
	...
	withoutField := bytes.Replace(data, []byte(`,"supervisor":"{forge@bee.hive.service:supervisor_host|0x1}"`), nil, 1)
	if _, err := Decode(withoutField); err != nil {
		t.Fatalf("descriptor without a supervisor address refused: %v", err)
	}
	for _, bad := range [][]byte{
		bytes.Replace(data, []byte(`"{forge@bee.hive.service:supervisor_host|0x1}"`), []byte(`null`), 1),
		append(data[:len(data)-1:len(data)-1], []byte(`,"supervisor":"{forge@bee.hive.service:supervisor_host|0x2}"}`)...),
		bytes.Replace(data, []byte(`"{forge@bee.hive.service:supervisor_host|0x1}"`), []byte(`"{other@bee.hive.service:supervisor_host|0x1}"`), 1),
	} {
