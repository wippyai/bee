...
local function supported(protocol: string): boolean
...
function M.binding(input: Input): Binding
    ...
    if decoded then
        ...
        for index, item in ipairs(decoded.profiles) do
            local ok = supported(item.protocol)
        ...
        if default and not supported(default.protocol) then fail("the default profile uses an unsupported protocol") end
