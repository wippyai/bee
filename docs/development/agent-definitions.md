...
| Bee child launch | `thread_launch` accepts a definition, brief and retry key, then checks the caller's policy and uses the caller's workspace and thread. A `function.lua` tool already fronts that path. (`modules/harness/src/launch/agent_launch.lua:9-29`, `modules/harness/src/launch/agent_launch_method.lua:44-85`, `modules/harness/src/launch/_index.yaml:83-99`) |
