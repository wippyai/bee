...
local function no_database()
    local handle, err = sql.get("bee:client_db")
...
local function main(mode: string)
    no_database()
    local list = {version = 1, database_resource = "bee:client_db"}
    local allocate = {version = 1, database_resource = "bee:client_db", desktop_id = string.rep("e", 32)}
    if mode == "none" then
    ...
    elseif mode == "reader" then
        ...
        call("allocate_desktop", {version = 1, database_resource = "bee:client_db", desktop_id = default}, "CONFLICT")
        ...
        call("list_desktops", {version = 1, database_resource = "bee:client_db", actor = "owner"}, "INVALID_ARGUMENT")
        call("allocate_desktop", {version = 1, database_resource = "bee:client_db", desktop_id = "bad"}, "INVALID_ARGUMENT")
        call("allocate_desktop", {version = 1, database_resource = "bee:client_db", desktop_id = allocate.desktop_id, label = "unexpected"}, "INVALID_ARGUMENT")
    end
    no_database()
end
return {main = main}
