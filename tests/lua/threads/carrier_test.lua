...
local function define_tests()
    test.describe("Thread carrier", function()
        ...
        test.it("records a durable cancel intent per attempt and surfaces it on the checkpoint", function()
            local thread_id = prepared_attempt()
            local before = harness.value(carrier:call("carrier_checkpoint", {thread_id = thread_id, attempt_id = "t1"}))
            test.is_nil(before.cancel_intent)
            local recorded = harness.value(carrier:call("carrier_cancel_intent", {thread_id = thread_id, attempt_id = "t1", idempotency_key = "cancel-1", state = "cancelling"}))
            test.eq(recorded.state, "cancelling")
            test.eq(recorded.attempt_id, "t1")
            test.eq(recorded.idempotency_key, "cancel-1")
            -- A second caller replays the same intent instead of replacing it.
            local replayed = harness.value(carrier:call("carrier_cancel_intent", {thread_id = thread_id, attempt_id = "t1", idempotency_key = "cancel-2", state = "cancelling"}))
            test.eq(replayed.state, "cancelling")
            ...
            test.eq(ended.state, "ended")
            test.eq(ended.outcome, "cancelled")
            local kept = harness.value(carrier:call("carrier_cancel_intent", {thread_id = thread_id, attempt_id = "t1", state = "cancelling"}))
            test.eq(kept.state, "ended")
            test.eq(kept.outcome, "cancelled")
            local db = harness.open()
            local rows = harness.query(db, "SELECT state, outcome FROM bee_thread_cancel_intents WHERE thread_id = ?", {thread_id})
            db:release()
            test.eq(#rows, 1)
            test.eq(rows[1].state, "ended")
            test.eq(rows[1].outcome, "cancelled")
        end)
        test.it("records a cancel intent before its attempt opens and reads it back", function()
            local thread_id = harness.thread(carrier, "Cancel before start")
            local early = harness.value(carrier:call("carrier_cancel_intent", {thread_id = thread_id, attempt_id = "t0", idempotency_key = "pre-key", state = "ended", outcome = "cancelled"}))
            test.eq(early.state, "ended")
            test.eq(early.outcome, "cancelled")
            local read = harness.value(carrier:call("carrier_cancel_status", {thread_id = thread_id, attempt_id = "t0"}))
            test.eq(read.state, "ended")
            test.eq(read.outcome, "cancelled")
            test.eq(read.idempotency_key, "pre-key")
            test.eq(harness.code(carrier:call("carrier_cancel_status", {thread_id = thread_id, attempt_id = "t9"})), "NOT_FOUND")
            test.eq(harness.code(runner:call("carrier_cancel_status", {thread_id = thread_id, attempt_id = "t0"})), "DENIED")
            -- A late carrier boot no longer opens an attempt settled before start.
            harness.value(carrier:call("admit_action", {thread_id = thread_id, idempotency_key = harness.key(), action_id = "a0", admitted = harness.admitted()}))
            test.eq(harness.code(carrier:call("prepare_attempt", {thread_id = thread_id, idempotency_key = harness.key(), action_id = "a0", attempt_id = "t0", prepared = harness.prepared()})), "CONFLICT")
            local kept = harness.value(carrier:call("carrier_cancel_status", {thread_id = thread_id, attempt_id = "t0"}))
            test.eq(kept.state, "ended")
            -- A merely cancelling intent still lets the attempt open.
            harness.value(carrier:call("carrier_cancel_intent", {thread_id = thread_id, attempt_id = "t1", state = "cancelling"}))
            harness.value(carrier:call("prepare_attempt", {thread_id = thread_id, idempotency_key = harness.key(), action_id = "a0", attempt_id = "t1", prepared = harness.prepared()}))
        end)
        test.it("refuses cancel intents outside its members and its typed boundary", function()
            local thread_id = prepared_attempt()
            test.eq(harness.code(runner:call("carrier_cancel_intent", {thread_id = thread_id, attempt_id = "t1", state = "cancelling"})), "DENIED")
            test.eq(harness.code(carrier:call("carrier_cancel_intent", {thread_id = thread_id, attempt_id = "t1", state = "stopped"})), "INVALID_ARGUMENT")
            test.eq(harness.code(carrier:call("carrier_cancel_intent", {thread_id = thread_id, attempt_id = "t1", state = "ended"})), "INVALID_ARGUMENT")
            test.eq(harness.code(carrier:call("carrier_cancel_intent", {thread_id = thread_id, attempt_id = "t1", state = "ended", outcome = "failed"})), "INVALID_ARGUMENT")
            test.eq(harness.code(carrier:call("carrier_cancel_intent", {thread_id = thread_id, attempt_id = "t1", state = "cancelling", outcome = "cancelled"})), "INVALID_ARGUMENT")
            test.eq(harness.code(carrier:call("carrier_cancel_intent", {thread_id = thread_id, attempt_id = "t1", state = "cancelling", extra = true})), "INVALID_ARGUMENT")
        end)
