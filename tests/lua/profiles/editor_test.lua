...
local function define_tests()
    test.describe("Saved profile editor", function()
        ...
        test.it("keeps a folder and a thread choice only where the launch allows the override", function()
            ...
            test.is_true(editor.set_workdir(open, "bee.environment:workspace_root", "legacy/app"))
            ...
            test.is_false(editor.set_workdir(open, "bee.environment:workspace_root", "../escape"))
            ...
            local closed = draft()
            local refused, refusal = editor.set_workdir(closed, "bee.environment:workspace_root", "legacy")
            ...
            local saved = protocol.profile({title = "Original", definition_ref = "bee:codex", options = {}, mcp_tools = {},
                instructions = "", workdir = {root_ref = "bee.environment:workspace_root", path = "legacy"}})
