...
local function main(value: unknown)
    ...
    local function refresh()
        ...
        for _, node in ipairs(state.nodes) do
            if node.member then
                if supervisor.running then model.apply_presence(state, node.node_id, source:presence(node.node_id))
    ...
    local function ask(mode: directory.Mode)
        ...
        if state.pending then act(mode); return end
    ...
    while running do
        ...
        elseif event.channel == ticks then
            if not state.pending and not dialog then refresh() end
        ...
        elseif event.channel == answers then
            ...
            if result and dialog and result.request_id == dialog.request_id then
                local asked = dialog
                dialog = nil
                if result.action == "accept" then act(asked.mode) else status = "Cancelled"; dirty = true end
            end
        else
            ...
            elseif data.type == "key" and data.action ~= "release" then
                ...
                elseif key == "enter" then
                    if state.pane == "nodes" then open_selected() else ask("observe") end
                elseif letter == "c" then ask("control")
                elseif letter == "o" then ask("observe")
                elseif letter == "r" then refresh()
