...
| `bee.hive.telemetry:cluster` | One holdings page from each named node (at most eight): a workspace count per reachable node, `unavailable` otherwise |
...
`unavailable`, never counted as holding nothing. Neither operation grants
anything or changes state.
