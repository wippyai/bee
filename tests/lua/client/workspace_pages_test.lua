...
local function define_tests()
    test.describe("Display workspace pages", function()
        test.it("decodes presenter queries strictly", function()
            ...
            test.is_nil(workspace_pages.query({version = 1, op = "workspaces", request_id = "r1", root_ref = "bee.env:workspace_root"}))
        end)
        test.it("decodes a catalog page and keeps the folder's unnamed row", function()
            local page = workspace_pages.decode({ok = true, value = {items = {{workspace_id = ID, label = "", root_ref = "bee.env:workspace_root"}},
                next_after = "cursor-2"}})
